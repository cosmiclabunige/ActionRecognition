...
def deep_network_test_and_tflite(CNNModel, inputSize, LSTMNeurons, DenseNeurons, nClasses):
    ...
    RNN = tf.keras.layers.RNN(tf.keras.layers.LSTMCell(LSTMNeurons))(TD)  # adding the LSTM layer
    Dense1 = tf.keras.layers.Dense(DenseNeurons, activation='relu')(RNN)  # adding the Dense layer
...
def deep_network_train(CNNModel, inputSize, LSTMNeurons, DenseNeurons, nClasses):
    ...
    RNN = tf.keras.layers.RNN(tf.keras.layers.LSTMCell(LSTMNeurons))(TD)  # adding the LSTM layer
    Dense1 = tf.keras.layers.Dense(DenseNeurons, activation='relu')(RNN)  # adding the Dense layer
    Dense2 = tf.keras.layers.Dense(nClasses, activation='softmax')(Dense1)  # last layer performs the classification of the input
    model_ = tf.keras.models.Model(inputs=input_shape, outputs=Dense2)
    model_.summary()
    return model_
...
def MobileNetV2(shape): # MobileNetV2 for transfer learning
    base_model = tf.keras.applications.MobileNetV2(input_shape=shape, include_top=False, pooling="avg")
...
def Xception(shape): # Xception for transfer learning
    base_model = tf.keras.applications.Xception(input_shape=shape, include_top=False, pooling="avg")
    base_model.trainable = False
    return base_model
...
def ResNet50V2(shape): # ResNet50 for transfer learning
    base_model = tf.keras.applications.ResNet50V2(input_shape=shape, include_top=False, pooling="avg")
    base_model.trainable = False
    return base_model
